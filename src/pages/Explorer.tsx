...
export function Explorer({ molecule, loading, error, onSearch }: ExplorerProps) {
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-white dark:bg-[#0B0E14] relative overflow-hidden">
        {/* Ambient background glow */}
        <div className="absolute inset-0 bg-blue-500/5 blur-[120px] rounded-full scale-50" />
        
        <div className="relative flex flex-col items-center">
          <div className="relative w-24 h-24 mb-8">
            {/* Spinning atomic orbitals */}
            <motion.div 
              animate={{ rotate: 360 }}
            ...
            <motion.div 
              ...
              className="absolute inset-2 border-2 border-dotted border-blue-400/20 rounded-full"
            />
            <motion.div 
            ...
            >
              <FlaskConical size={40} className="drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]" />
            </motion.div>
          </div>
          ...
          >
            <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Compound Analysis</h2>
            <div className="flex items-center gap-2 justify-center">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
              <p className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Synthesizing 3D Structure...</p>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-600 max-w-xs mx-auto italic">
              Retrieving structural conformers from scientific session logs.
            </p>
          </motion.div>
        </div>
      </div>
  ...
  if (error || !molecule) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-white dark:bg-[#0B0E14] text-slate-400 px-6 text-center">
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row bg-white dark:bg-[#0B0E14] relative overflow-hidden transition-colors duration-300">
      ...
      {!isSidebarOpen && (
        <motion.button 
          ...
          className="absolute top-6 left-6 z-40 p-3 bg-white/80 dark:bg-[#161B22]/80 backdrop-blur-md border border-slate-200 dark:border-white/10 rounded-xl text-slate-900 dark:text-white shadow-2xl hover:bg-slate-50 dark:hover:bg-[#1C2128] transition-all"
        >
          <Menu size={20} />
        </motion.button>
      )}
      ...
      <AnimatePresence mode="wait">
        {isSidebarOpen && (
          <motion.aside 
            ...
            className="fixed lg:relative inset-y-0 left-0 w-80 bg-slate-50 dark:bg-[#161B22] border-r border-slate-200 dark:border-slate-800 flex flex-col shrink-0 z-40 lg:z-20 shadow-2xl lg:shadow-none transition-colors"
          >
            <div className="p-6 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Compound Explorer</span>
                </div>
                <button 
                  onClick={() => setIsSidebarOpen(false)}
                  className="p-2 text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors bg-white dark:bg-[#0B0E14] rounded-lg border border-slate-200 dark:border-slate-800/50"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 bg-white dark:bg-[#0B0E14] rounded-xl border border-slate-200 dark:border-slate-800">
                  <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase mb-1">Molar Mass</p>
                  <p className="text-xs font-black text-slate-700 dark:text-slate-200">{molecule.molecularWeight} <span className="text-[10px] font-medium opacity-60">g/mol</span></p>
                </div>
                <div className="p-3 bg-white dark:bg-[#0B0E14] rounded-xl border border-slate-200 dark:border-slate-800">
                  <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase mb-1">Charge</p>
                  <p className="text-xs font-black text-slate-700 dark:text-slate-200">{molecule.properties.Charge || 0}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <section>
                ...
                <div className="p-4 bg-white dark:bg-[#0B0E14] rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4">
                  <div className="flex items-center justify-between gap-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-lg">
                    <button 
                      onClick={() => setCalcDirection('mass-to-moles')}
                      className={`flex-1 text-[9px] font-black py-1.5 rounded-md transition-all ${calcDirection === 'mass-to-moles' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500'}`}
                    >
                      MASS → MOLES
                    </button>
                    <button 
                      onClick={() => setCalcDirection('moles-to-mass')}
                      className={`flex-1 text-[9px] font-black py-1.5 rounded-md transition-all ${calcDirection === 'moles-to-mass' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500'}`}
                    >
                      MOLES → MASS
                    </button>
                  </div>
                    ...
                    {calcDirection === 'mass-to-moles' ? (
                      <>
                        <div className="space-y-1">
                          <label className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase ml-1">Input Mass (g)</label>
                          <input 
                            type="number"
                            value={calcMass}
                            onChange={(e) => {
                            ...
                            className="w-full bg-slate-50 dark:bg-[#161B22] border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white focus:border-blue-500 focus:outline-none"
                            placeholder="0.00"
                          />
                        </div>
                        <div className="pt-2 border-t border-slate-200 dark:border-slate-800/50 flex justify-between items-center">
                          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500">Result:</span>
                          <span className="text-xs font-black text-blue-600 dark:text-blue-400">{calcMoles || '0.0000'} <span className="text-[9px] font-medium opacity-60">mol</span></span>
                        </div>
                      </>
                    ) : (
                      <>
                        <div className="space-y-1">
                          <label className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase ml-1">Input Moles (mol)</label>
                          <input 
                            type="number"
                            value={calcMoles}
                            onChange={(e) => {
                        ...
                        <div className="pt-2 border-t border-slate-200 dark:border-slate-800/50 flex justify-between items-center">
                          <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500">Result:</span>
                          <span className="text-xs font-black text-emerald-600 dark:text-emerald-400">{calcMass || '0.0000'} <span className="text-[9px] font-medium opacity-60">g</span></span>
                        </div>
                      </>
                    )}
                  </div>
              ...
              <section>
                ...
                <div className="flex flex-col gap-2">
                  <button 
                    ...
                    className="w-full bg-white dark:bg-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-bold py-2.5 rounded-lg transition-all border border-slate-200 dark:border-slate-700 flex items-center justify-center gap-2"
                  >
              ...
              <section>
                ...
                <div className="flex flex-col gap-2">
                  <button 
                    ...
                    className="w-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-bold py-2.5 rounded-lg transition-all border border-slate-200 dark:border-slate-700 flex items-center justify-center gap-2"
                  >
                    <Settings size={16} />
                    Detailed Properties
                  </button>
                  ...
                  <button 
                    ...
                    className={`w-full text-sm font-medium py-2 rounded-lg transition-colors border ${
                      isFavorite 
                        ...
                        : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border-slate-200 dark:border-slate-800'
      ...
      <section className="flex-1 flex flex-col relative h-full">
        ...
        {/* View Tabs */}
        <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-white/80 dark:bg-[#161B22]/80 backdrop-blur-md p-1 rounded-xl border border-slate-200 dark:border-white/5 flex items-center gap-1 z-30 shadow-2xl">
          {[
        ...
        <div className="flex-1 relative overflow-hidden">
          {activeTab === '3d' ? (
            molecule.structure3d ? (
              ...
              <div className="h-full flex flex-col items-center justify-center p-8 text-slate-400 bg-white dark:bg-[#0B0E14] overflow-y-auto">
                <div className="max-w-md w-full text-center space-y-8">
                  <div>
                    ...
                    <h3 className="text-xl font-black text-slate-900 dark:text-white">3D Dataset Not Available</h3>
                  ...
                  {molecule.cid && (
                    <motion.div 
                      ...
                      className="bg-slate-50 dark:bg-[#161B22] p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden group"
                    >
                      <span className="text-[10px] font-black text-blue-600 dark:text-blue-500 uppercase tracking-widest mb-6 block">Structural 2D Representation</span>
                      ...
                      <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800/50 flex flex-col items-center">
                        <span className="text-[9px] font-bold text-slate-400 dark:text-slate-600 uppercase mb-2">Canonical SMILES</span>
                        <p className="text-xs font-mono text-slate-600 dark:text-slate-400 break-all bg-white dark:bg-[#0B0E14] p-3 rounded-xl border border-slate-200 dark:border-slate-800/50 w-full">
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-500 bg-white dark:bg-[#0B0E14]">
            ...
            <div className="h-full p-8 overflow-y-auto bg-white dark:bg-[#0B0E14]">
              <div className="max-w-2xl mx-auto space-y-8 pt-20">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(molecule.properties).map(([key, value]) => (
                    <div key={key} className="p-4 bg-slate-50 dark:bg-[#161B22] border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col gap-1">
                      <span className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                      <span className="text-sm font-bold text-slate-900 dark:text-slate-200">{String(value)}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {/* Interaction Strip */}
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-white/90 dark:bg-[#161B22]/90 backdrop-blur-md px-6 py-4 rounded-2xl border border-slate-200 dark:border-white/5 z-30 shadow-2xl">
          <div className="flex items-center gap-2 pr-4 border-r border-slate-200 dark:border-white/5">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Live Terminal</span>
          </div>
          <div className="flex items-center gap-3">
             <button 
               onClick={handleSnapshot}
      ...
      <AnimatePresence>
        {isPropertiesOpen && (
          <motion.div
            ...
            className="fixed inset-y-0 right-0 w-full sm:w-96 bg-white dark:bg-[#161B22] border-l border-slate-200 dark:border-slate-800 z-50 flex flex-col shadow-[-20px_0_40px_rgba(0,0,0,0.4)] transition-colors"
            drag="x"
          ...
          >
            <div 
              className="h-10 w-full flex items-center justify-center border-b border-slate-100 dark:border-slate-800 cursor-grab active:cursor-grabbing text-slate-300 dark:text-slate-700 hover:text-slate-500 transition-colors"
            ...
            <div className="p-6 flex items-center justify-between border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-black text-slate-900 dark:text-white">Physical Metrics</h3>
              </div>
              <button 
              ...
              <section className="p-4 bg-blue-500/5 rounded-2xl border border-blue-500/10">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-slate-600 dark:text-slate-300 font-mono">{molecule.cid || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500">Isomeric SMILES</span>
                    <span className="text-slate-600 dark:text-slate-300 font-mono truncate max-w-[150px]" title={molecule.smiles}>{molecule.smiles || 'N/A'}</span>
                  </div>
                </div>
              </section>
            </div>

            <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#0B0E14]/50">
              <button 
                onClick={() => generateMoleculeReport(molecule)}
          ...
          {uniqueElements.map(symbol => {
            ...
            return (
              <div key={symbol} className="flex items-center gap-3">
                ...
                />
                <span className="text-[10px] font-black text-slate-600 dark:text-slate-300">{symbol}</span>
                <span className="text-[8px] font-medium text-slate-400 dark:text-slate-600 uppercase ml-auto">
...
function ToggleButton({ active, onClick, label }: { active: boolean, onClick: () => void, label: string }) {
  return (
    <button 
      onClick={onClick}
      className="flex items-center justify-between gap-4 px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group"
    >
      <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">{label}</span>
      <div className={`w-8 h-4 rounded-full relative transition-colors ${active ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-600'}`}>
        <motion.div 
...
function MetricCard({ label, value }: { label: string, value: string | number }) {
  return (
    <div className="p-3 bg-slate-50 dark:bg-[#0B0E14] rounded-xl border border-slate-200 dark:border-slate-800">
      <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase mb-1">{label}</p>
