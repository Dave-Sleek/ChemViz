...
export function Help() {
  ...
  return (
    <div className="min-h-full bg-slate-50 dark:bg-[#0B0E14] transition-colors duration-300 p-8 overflow-y-auto">
      <div className="max-w-4xl mx-auto">
        <header className="mb-16">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase tracking-widest mb-4"
          >
            <BookOpen size={12} />
            Laboratory Manual
          </motion.div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight mb-4">Documentation & Help</h1>
          <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl">Everything you need to master molecular visualization and chemical analysis in the Molecufy environment.</p>
        </header>
        ...
        <div className="grid gap-8">
          {sections.map((section, idx) => (
            <motion.div 
              ...
              className="bg-white dark:bg-[#161B22] border border-slate-200 dark:border-slate-800 rounded-[2rem] p-8 shadow-xl"
            >
              <div className="flex items-start gap-6">
                <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-[#0B0E14] border border-slate-200 dark:border-slate-800 flex items-center justify-center shrink-0">
                  {section.icon}
                </div>
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">{section.title}</h3>
                  <p className="text-slate-500 dark:text-slate-400 leading-relaxed mb-6">{section.content}</p>
                  
                  <div className="bg-slate-50 dark:bg-[#0B0E14] rounded-2xl p-6 border border-slate-100 dark:border-slate-800/50">
                    <h4 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Pro Tips</h4>
        ...
        <footer className="mt-20 pt-12 border-t border-slate-200 dark:border-slate-800 flex flex-col items-center text-center">
          <div className="w-16 h-16 bg-blue-500/10 rounded-3xl flex items-center justify-center text-blue-500 mb-6">
            <Cpu size={32} />
          </div>
          <h3 className="text-slate-900 dark:text-white font-bold mb-2">Technical Support</h3>
          <p className="text-slate-500 dark:text-slate-500 text-sm max-w-sm">Molecufy is an educational platform. For complex chemical queries, always consult peer-reviewed scientific literature.</p>
        </footer>
      </div>
    </div>
  );
