...
export async function fetchMoleculeByFormula(input: string): Promise<MoleculeData | null> {
  try {
    ...
    // Use listkey for larger results if needed, but for common ones CID is fine
    const cidResponse = await fetch(`${PUBCHEM_BASE_URL}/compound/name/${encodeURIComponent(input)}/cids/JSON`);
    const cidData = await cidResponse.json();
    
    if (!cidData.IdentifierList?.CID?.[0]) {
      // Try by formula if name fails
      const cidByFormulaResponse = await fetch(`${PUBCHEM_BASE_URL}/compound/fastformula/${encodeURIComponent(input)}/cids/JSON`);
      const cidByFormulaData = await cidByFormulaResponse.json();
      if (!cidByFormulaData.IdentifierList?.CID?.[0]) {
        // Fallback to local parsing if PubChem fails
...
async function fetchMoleculeByCID(cid: number, originalInput: string): Promise<MoleculeData | null> {
  try {
    // 2. Get properties
    const propsResponse = await fetch(`${PUBCHEM_BASE_URL}/compound/cid/${cid}/property/MolecularFormula,MolecularWeight,CanonicalSMILES,IUPACName,Complexity,Charge,HBondDonorCount,HBondAcceptorCount,RotatableBondCount,ExactMass/JSON`);
    ...
    try {
      ...
      description = descData.InformationList.Information[0].Description || descData.InformationList.Information[1].Description || '';
    } catch (e) { /* ignore */ }
      ...
      if (conformerData.PC_Compounds?.[0]) {
        ...
        structure3d = {
          atoms: conformerAtoms.element.map((el: number, idx: number) => {
            // PubChem element is atomic number
